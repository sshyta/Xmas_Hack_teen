...
# Загрузка данных
data_folder = 'data'
wifi_logs = pd.read_csv(os.path.join(data_folder, 'wifi_logs.csv'), delimiter=';')
...
# Преобразование 'mean_time' в секунды
movement_matrix['mean_time'] = (movement_matrix['mean_time'] - movement_matrix['mean_time'].min()).dt.total_seconds()

# Создание матриц перемещений
matrix_day = movement_matrix[movement_matrix['time_interval'] == 'Day'].pivot_table(values='mean_time', index='router_mac', columns='router_id', fill_value=0)
matrix_morning_peak = movement_matrix[movement_matrix['time_interval'] == 'Morning Peak'].pivot_table(values='mean_time', index='router_mac', columns='router_id', fill_value=0)
matrix_evening_peak = movement_matrix[movement_matrix['time_interval'] == 'Evening Peak'].pivot_table(values='mean_time', index='router_mac', columns='router_id', fill_value=0)

# Визуализация матриц перемещений
def visualize_movement_matrix(matrix, title):
    plt.figure(figsize=(12, 10))
    sns.heatmap(matrix, cmap="YlGnBu", annot=True, fmt=".2f", linewidths=.5, mask=matrix == 0, annot_kws={"size": 8})
    plt.title(title)
